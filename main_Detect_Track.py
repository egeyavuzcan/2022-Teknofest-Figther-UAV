...
### Input
#VideoInput = 'Input/SUFAI_Savasan_IHA_2021_Kilitlenmeler.mp4'  #Input video
VideoInput = 0  #Input video
...
doubleCheckThres = 0.85                 # If below thresh. => Double check if detection was accurate in next frame.
trackingConfidenceThres = 0.2           # Threshold for continuing tracking
VerificationInterval = 0.05 #sec        # Verification time interval in seconds of the tracking

### Flags
Initial_State = "Detection"
show_output = True                      # True: plots output
save_output = True                     # True: saves output in output folder
...
ImportType = 'None'                    # Input video import type"JetsonCam", "Queue", "Pre-load", None(= cv2 VideoCapture)

### Models
cfg = 'yolo/model/yolov3_dvr.cfg' 
data = 'yolo/model/yolo-drone.data' 
weights = 'yolo/model/3000.weights' 
